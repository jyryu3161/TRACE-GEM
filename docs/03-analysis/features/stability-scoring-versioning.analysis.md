...
| 12 | `src/versioning/change_summarizer.py` -- `ChangeSummarizer` with `summarize()`, `_template_summary()` | design:292-323 | `src/versioning/change_summarizer.py` | MATCH | `ChangeSummarizer(config)` with `async summarize(diff, change_type)` and `_template_summary(diff, change_type)`. Template-only summaries. All 4 change types handled: `initial_load`, `gap_fill`, `manual_edit`, `restore`. |
